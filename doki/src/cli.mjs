...
try{
  if(cmd==='rebuild'){
    ...
    try{for(const table of ['dialog_messages','prompt_runs','phase_reports','anomalies','gaps','observations','update_jobs','rotation_state','q_table']) ddb.exec(`DELETE FROM ${table}`); ddb.exec('COMMIT');}
    catch(error){try{ddb.exec('ROLLBACK')}catch{};throw error;}
  }
  const events=eventId?[{id:eventId}]:listTerminalEvents(fdb);
  for(const e of events) console.log(JSON.stringify(await processEvent({falsifyDb:fdb,dokiDb:ddb,eventId:e.id})));
