...
function fixture(activeState='DONE'){
  ...
  fdb.prepare('INSERT INTO jobs VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)').run('j1',null,null,activeState,activeState==='DONE'?'DONE WRITE':'RUNNING','WRITE','scan',1,1,5,null,null,0,'h','c',null,'2026-09-03T00:00:00Z','2026-09-03T00:01:00Z','2026-09-03T00:02:00Z');
  fdb.prepare('INSERT INTO loop_events VALUES(?,?,?,?,?,?,?,?,?,?)').run('e1','j1','s1',null,'c','transition','RUNNING',activeState,JSON.stringify({safe:true}),'2026-09-03T00:02:00Z');
...
test('read-only FalsifyMe DB rejects writes',()=>{const {dir,fPath,fdb}=fixture();fdb.close();const db=openReadOnlyFalsifyDb(fPath);assert.throws(()=>db.exec("INSERT INTO scopes VALUES('x','','','','','','',NULL)"));db.close();rmSync(dir,{recursive:true,force:true});});

test('active thinker closes the shared key window',()=>{const {dir,fdb}=fixture('RUNNING');assert.equal(sharedKeyWindowOpen(fdb,'e1'),false);fdb.close();rmSync(dir,{recursive:true,force:true});});
