...
import { config } from '../src/model.mjs';
import { narrativeAnalysis } from '../src/history.mjs';
...
function fixture(activeState='DONE') {
  ...
  fdb.exec(`
    ...
    CREATE TABLE checkouts(
      checkout_id TEXT PRIMARY KEY, project_id TEXT, bound_root TEXT, root_name TEXT,
  ...
  fdb.prepare(`INSERT INTO jobs (
    ...
    'j1', null, 's1', null, null, null, null, null, null, 'scan', 'write',
    activeState === 'DONE' ? 'DONE WRITE' : 'RUNNING', 'WRITE', null, null, null, 1, 2,
    null, null, '2026-09-03T00:00:00Z', '2026-09-03T00:01:00Z', '2026-09-03T00:02:00Z',
  ...
  fdb.prepare(`INSERT INTO scopes(
    id,checkout_id,header,status,phase,last_befund,sub_prompt,open_conflicts,hardened_at,
    created_at,updated_at,done_at,last_gap,last_divergence,research_additions
  ) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`).run(
    's1', null, 'fix parser bug', 'done', 'write', 'facts', null, 0, null,
...
test('model config uses the shared Thinker key and one output model', () => {
  const cfg = config({ DOKI_API_BASE: 'https://example.invalid', DOKI_THINKER_API_KEY: 'shared', DOKI_THINKER_MODEL: 'thinker' });
  assert.equal(cfg.key, 'shared');
  assert.equal(cfg.model, 'thinker');
});

test('narrative analysis is local and deterministic', () => {
  const { dir, fdb } = fixture();
  const snapshot = { loop_event: { id: 'e1', event_type: 'transition', from_state: 'RUNNING', to_state: 'DONE', job_id: 'j1' }, job: { verdict: 'WRITE', loop_state: 'DONE', files: ['a.mjs'] }, scope: { header: 'fix parser bug' }, findings: [{ wave: 'scan', verdict: 'WRITE', befund: 'repair parser' }, { wave: 'evil', verdict: 'WRITE', befund: 'attack parser' }] };
  const a = narrativeAnalysis(snapshot, { refs: [], historyDigest: 'h1' });
  const b = narrativeAnalysis(snapshot, { refs: [], historyDigest: 'h1' });
  assert.deepEqual(a, b);
  assert.ok(a.narrator);
  assert.ok(a.mood);
  assert.ok(a.matches.length > 0);
  fdb.close(); rmSync(dir, { recursive: true, force: true });
});
...
test('active job closes the shared key window', () => {
  const { dir, fdb } = fixture('RUNNING');
  assert.equal(sharedKeyWindowOpen(fdb, 'e1'), false);
  fdb.close(); rmSync(dir, { recursive: true, force: true });
});

test('doki db contains independent narrative state', () => {
  const { dir, fdb, dPath } = fixture();
  fdb.close();
  ...
  for (const n of ['observations', 'update_jobs', 'phase_reports', 'gaps', 'prompt_runs', 'dialog_messages', 'rotation_state', 'anomalies', 'persona_state', 'persona_relationships']) assert.ok(names.includes(n));
  db.close(); rmSync(dir, { recursive: true, force: true });
});
