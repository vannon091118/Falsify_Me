...
export async function runModels(args = []) {
  ...
  for (let i = 0; i < args.length; i += 1) {
    if (args[i] === "--api-base") options.apiBase = args[++i];
      ...
      console.error(`FEHLER: Unbekannte models-Option: ${args[i]}`);
      process.exit(2);
    }
  }
  try {
    ...
    console.error(`FEHLER: Modelle nicht erreichbar (${options.apiBase || getRuntimeSettings().apiBase}): ${e.message}`);
    process.exit(3);
  }
}
